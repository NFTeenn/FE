...
export default function ChatBotModal({ onClose }: { onClose: () => void }) {
	...
	return (
		<>
			{/* 배경 오버레이 */}
			<button
				className={`fixed inset-0 bg-black transition-opacity duration-300 z-40 ${isClosing ? "opacity-0" : isOpening ? "opacity-0" : "opacity-50"
					}`}
				onClick={handleClose}
			/>
			...
			{/* 챗봇 사이드바 */}
			<div
				className={`fixed top-0 right-0 h-full w-[400px] bg-white shadow-lg border-l border-gray-300 flex flex-col z-50 transition-transform duration-300 ${isClosing ? "translate-x-full" : isOpening ? "translate-x-full" : "translate-x-0"
					}`}
			>
