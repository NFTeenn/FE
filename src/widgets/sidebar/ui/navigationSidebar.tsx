				...
				<ul className="flex flex-col gap-4 w-full items-center">
					{menuItems.map((item) => (
						<Link
							key={item.name}
							href={item.path}
							className={`w-full h-12 flex justify-center items-center rounded-xl cursor-pointer hover:bg-gray-100 transition-colors px-2 ${pathname === item.path ? "bg-gray-100" : ""}`}
						>
							<span
								className={
									pathname === item.path
