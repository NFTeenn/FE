...
const EconomicNews = () => {
  ...
  const fetchNews = async (searchQuery: string): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : '뉴스를 불러오는데 실패했습니다';
      setError(errorMessage);
      // 데모용 샘플 데이터
      setNews([
        {
          title: '한국은행, 기준금리 동결 결정',
          description: '한국은행이 기준금리를 3.5%로 유지하기로 결정했습니다. 물가 안정과 경제 성장을 고려한 결정입니다.',
          link: '#',
          pubDate: new Date().toISOString()
        },
        {
          title: '코스피 2500선 돌파, 증시 상승세',
          description: '코스피 지수가 2500선을 돌파하며 강한 상승세를 보이고 있습니다.',
          link: '#',
          pubDate: new Date().toISOString()
        },
        {
          title: '부동산 시장 안정화 조짐',
          description: '수도권 부동산 시장이 안정화 조짐을 보이면서 거래량이 증가하고 있습니다.',
          link: '#',
          pubDate: new Date().toISOString()
        }
      ]);
      ...
      {error && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <p className="text-yellow-800">
            ⚠️ {error}
            <br />
            <span className="text-sm">샘플 데이터를 표시합니다. 실제 사용 시 백엔드 서버를 통해 API를 호출하세요.</span>
          </p>
        </div>
      )}
