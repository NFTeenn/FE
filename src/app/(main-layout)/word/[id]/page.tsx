...
export default function WordPage({ params }: { params: { id: string } }) {
	...
	return (
		<div className="flex flex-col gap-4 px-24 py-16">
			...
			<article
				key={word.num}
				className="flex justify-between items-center gap-8 bg-white rounded-xl p-12 border border-black/20"
			>
				...
				<button className="bg-brand-b1 text-white text-xl font-semibold py-4 px-11 rounded-lg cursor-pointer">
					AI로 요약하기
				</button>
			</article>
			<article
			...
			>
				...
				<div className="grid grid-cols-4 gap-4">
					<article
						key={word.num}
						className="flex items-center gap-8 bg-white rounded-md px-5 py-4 border border-black/20"
					>
						<p>유의어</p>
					</article>
				</div>
